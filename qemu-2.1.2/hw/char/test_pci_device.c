...
static uint64_t test_pci_mmio_read(void *opaque, hwaddr addr, unsigned size)
{
...
static uint64_t test_pci_pio_read(void *opaque, hwaddr addr, unsigned size)
{
    TestPCIState *s = opaque;
		tprintf("addr %ld, size %d\n", addr, size);
...
static void test_pci_mmio_write(void *opaque, hwaddr addr, uint64_t val, unsigned size)
{
    TestPCIState *s = opaque;
...
static void test_pci_pio_write(void *opaque, hwaddr addr, uint64_t val, unsigned size)
{
    TestPCIState *s = opaque;
		PCIDevice *pdev = PCI_DEVICE(s);
...
static void test_pci_uninit(PCIDevice *pdev)
{
    TestPCIState *s = TEST_PCI(pdev);
...
static void test_pci_reset(TestPCIState *s)
{
    tprintf("done reset\n");
}
